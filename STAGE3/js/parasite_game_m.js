...
//    player.name = playerName == "" ? "Anonymous" : playerName;
    isStarted = true;
    startTime = new Date().getTime(); // 타이머
    initialiseMap();
    console.log("bbbb");
    document.getElementById("StartButton").disabled = false;
    document.getElementById("SplashScreen").style.display = "none";
...
var PATH_CHAR = "img/sprite_sheet2.png";

var CHAR_WIDTH = 60,
    CHAR_HEIGHT = 80,
    IMAGE_START_EAST_Y = 80,
    IMAGE_START_WEST_Y = 240,
    SPRITE_WIDTH = 180;

var COIN_WIDTH = 30,
    COIN_HEIGHT = 32,
    COIN_SPRITE_WIDTH = 120;

var FLYING_MONSTER_WIDTH = 32,
    FLYING_MONSTER_HEIGHT = 572,
...
monsterImage.src = "img/monster_sprite2.png";

var goodThingImage = new Image();
goodThingImage.src = "img/coin_sprite.png";

var tileImage = new Image();
tileImage.src = "img/tile_sprite.png";
...
projectileImage.src = "img/projectile_sprite.png";

var flyingMonsterImage = new Image();
flyingMonsterImage.src = "img/bat_sprite.png";
...
//
//        if(velX < 0 && velX < -1.5) flyingMonster.velX += 1.5;
//        if(velX > 0 && velX > 1.5) flyingMonster.velX -= 1.5;
//        if(velY < 0 && velY < -1.5) flyingMonster.velY += 1.5;
//        if(velY > 0 && velY > 1.5) flyingMonster.velY -= 1.5;
//
...
function drawFlyingMonster() {
     FLYING_MONSTER_HEIGHT = elapsedTime;
    if(flyingMonster != null) {
        ...
        FLYING_MONSTER_WIDTH, FLYING_MONSTER_HEIGHT, flyingMonster.x-5, flyingMonster.y-24,
...
//3초 있다가 who_is_he?로 넘어감 
setTimeout("location.href='../01/who_is_he.html'",3000);
    ...
    ctx.fillStyle = "white";
    ctx.font = "40px Arial";
    ctx.fillText("GAME CLEAR!",  600, 360);
        player.x = 1261;
    var table = getHighScoreTable();
    //ctx.fillStyle = "";
    ...
    ctx.font = "15px Arial";
    ctx.fillStyle = "white";
    ctx.fillText("3초 뒤 'WHO IS HE?'로 이동합니다.", 620, 420);
    ctx.fillText("다시 시작하려면 F5를 눌러주세요.", 620, 460);
    ctx.font = "0px Arial";
...
function drawExit() {
    if(goodThings.length == 0 ) {
        ...
        ctx.drawImage(exitImage,0,0, 116,126,
            exit.x,exit.y-100, 116,126);
    }
}
...
function drawGUI() {
    ...
    elapsedTime = parseInt((new Date().getTime()-startTime)/15, 10);
    remainingTime = 9000 - elapsedTime;
